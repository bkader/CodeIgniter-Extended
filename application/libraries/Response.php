...
class Response {
    ...
    {
        if ( ! $this->CI->input->get_post('_dialog_confirmed'))
        {
        ...
        // Destroy dialog
        $dialog_id = $this->CI->input->get_post('_dialog_id');
    ...
    {
        $this->CI->load->library('dialog');

        $dialog_id = (empty($data['id'])) ?
                'dialog-' . mt_rand(1000000, 9999999) :
                $data['id'];
        $this->CI->dialog->set_id($dialog_id);

        if ( ! empty($data['title']))
        {
            $this->CI->dialog->set_title($data['title']);
        }

        if ( ! empty($data['body']))
        {
            $this->CI->dialog->set_body($data['body']);
        }

        if ( ! empty($data['content']))
        {
            $this->CI->dialog->set_content($data['content']);
        }
        ...
        if ( ! empty($data['buttons']))
        {
            ...
            {
                $this->CI->dialog->add_button($type, $label);
            }
        }

        if ( ! empty($data['footer']))
        {
            $this->CI->dialog->set_footer($data['footer']);
        }

        if ( ! empty($data['size']))
        {
            $this->CI->dialog->set_size($data['size']);
        }

        $html = $this->CI->dialog->html();
